...
file in the \fBbin\fP field, so it is available to run by \fBname\fP or \fBname\.cmd\fP (on
...
either directly by \fBnpm exec\fP or by name in other scripts when invoking them 
via \fBnpm run\-script\fP\|\.
.P
...
from the \fBcli\.js\fP script to \fB/usr/local/bin/myapp\fP and in case of windows it 
will create a cmd file usually at \fBC:\\Users\\{Username}\\AppData\\Roaming\\npm\\myapp\.cmd\fP
which runs the \fBcli\.js\fP script\. 
.P
...
\fBengine\-strict\fP config \fI/using\-npm/config#engine\-strict\fR flag, this field is
